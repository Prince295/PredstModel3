...
class MainWidget():
    def __init__(self, parent = None):
        ...
        button[5].config(text = "Поиск по названию", command = self.find_names)
    ...
    def watch_collection(self):
        ...
        var2.set( self.scrolledList.listbox.get( 'active' ) )
        ...
        #     self.text_message = ThemedOut( form2 )
        #
        #     self.text_message.config( text=var2.get() )

        # var2.trace('w', callback1)
    ...
    def find_names(self):
        ...
        container2 = Container( form2 )
        button1 = CommandButton( container1 )
        button1.config( text='Найти', command=lambda: self.find_slot( entry, toplevel ) )
        button2 = CommandButton( container2 )
        button2.config( text='Отмена', command=lambda: toplevel.destroy() )
    ...
    def find_slot(self,array,toplevel):
        print(self.collection)
        picture = Picture( array[0].get(),
                           Author( array[3].get(), array[4].get(), array[5].get() ),
                           array[1].get(),
